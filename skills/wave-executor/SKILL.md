...
**CONTRACT PACK LOADING CONVENTION (build-contract-pack consumer rule, 2026-09-12):** When the initiative's repo contains `<repo-root>/.sisyphus/contract-pack/` (generated by the build-contract-pack skill at the plan/wave boundary), implementation slices touching a contracted component MUST: (1) read the component's contract in `component-contracts/` BEFORE building; (2) verify the result against the contract's Invariants AFTER building; (3) report deviations per the escalation rules in `fidelity-index.md` (read the pack root at the FIRST contracted slice of a wave) — stop-immediately classes (behavior/visual/token/API deviations) map to Rule 4 above; logged classes (unconstrained implementation details) track like Rules 1–3. **Validation-output requirement:** every contracted slice's validation output MUST include the deviation report or an explicit "no deviations" line — regression-gate and orchestrator-review treat its absence as a finding. No pack in the repo → this convention is inert.
