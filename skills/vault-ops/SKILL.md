...
Codified: execution form = `task(subagent_type="archivist")`; in-session role-form = documented fallback only, with the full procedural envelope; every execution receipt records the form used. Verbatim-quote discipline applies: any rewording of the clause = re-arbitration, not editorial.
