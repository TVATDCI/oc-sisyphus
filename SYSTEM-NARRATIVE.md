...
- **oh-my-openagent:** 4.19.4 (exact pin, no caret, auto_update: false; upgraded from 4.18.2 on Aug 10 — crossed the Jul 17 stop-short line after scoping showed zero `shared/`-alias and zero ralph references in repo files; verify-plugin-compat + 648 unit + 22 self-test PASS, delegate-task smoke OK)
...
- **2026-08-15 (session 2)** — herdr workspace recipes shipped to dotfiles (`41c4985` + pycache-cleanup `242e314`, operator-committed + pushed): `herdr/recipes/` = topology.py (live workspace→tab→pane→agent tree) + watchdog.py (pane run → wait-output → read tail gate) + approval_relay.py (agent wait --until blocked → notification show --sound request) + `_lib.py` (argv-list herdr wrapper; `raw=` for pane read; empty-stdout tolerated for pane run — both API quirks found by live testing). All verified against herdr 0.8.0 in-session (topology full run; watchdog e2e on scratch pane w3:p4 + timeout negative; relay unknown-agent negative; scratch pane closed). Toast `[ui.toast] delivery off→system` + `herdr server reload-config` → **operator-verified end-to-end in GNOME tray**; fullscreen suppresses banner popups by GNOME policy (tray still receives — operator's actual surface). pi's 2 proposed bd facts promoted (`skills_cli_shared_store`, `herdr_update_runbook`); stale pi exports-commit parked item struck (`21d42af` landed, verified via git log --stat). COMPLETE-CODEBASE check: no update needed (dotfiles-side tooling; no opencode skills/agents/routing/scripts touched). Evidence: `~/.sisyphus/evidence/session-close-2026-08-15-herdr-recipes-toast.md`.

- **2026-08-17** — Main-vault sharing policy A+C RATIFIED + implemented (pi⇄sis herdr-collab lane 2): pi read-only vault QUERY (bd `mainvault_pi_read_policy`) + operator-arbitrated C-path writes via archivist — proven end-to-end run 1 (Main-vault/AGENTS.md QUERY lane-partner note, commit `cfb6c18`, git-diff byte-verified). Oracle verdict SOUND-WITH-CONDITIONS, 9 conditions adopted (log-tail currency rule — vault semantic layer measured frozen since ~June). Watch-items filed: brain-ecf (no permission prompt on non-allowlisted edit), brain-avm (bridge cron uninstalled). Evidence: `~/.sisyphus/evidence/session-close-2026-08-17-mainvault-sharing-lane.md`.
