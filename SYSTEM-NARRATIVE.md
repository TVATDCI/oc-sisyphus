...
- **2026-08-15** — First live herdr-collab lane (pi w3:p1 ⇄ sis w3:p2): sis reviewed pi's cross-agent protocol skill 3 rounds — v0.1 (4 risks + 5 improvements) → v0.2 verified + SIGNED OFF → v0.2.1 (nits folded). Ratified: file-exchange-primary `/tmp/herdr-collab/<topic>/`, lane name `sis`, operator-owned focus, review `--timeout ≥300000`, symmetric ~/.pi//trust-root read restraint; empirical gate facts (bare herdr passes, substring metachar defense, bare `rm` blocked) now in the skill's gate-notes section. Evidence: `~/.sisyphus/evidence/session-close-2026-08-15-herdr-collab-lane.md`.
